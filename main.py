...
MAX_CONCURRENT_REQUESTS = 25          # reduced for reliability vs. 418s
HTTP_TIMEOUT = 15
RETRY_ATTEMPTS = 3
CANDLE_LIMIT = 200                    # more bars -> stabler indicators

# scan only top N by 24h quote volume (set to None to scan all)
TOP_N_BY_VOLUME = 150

# Thresholds (same for both sides)
MIN_SCORE_EARLY = 3
...
# === Binance hosts / headers (keep-alive + failover) ===
BINANCE_FAPI_HOSTS = [
...
async def fetch_json(session, url):
    """GET with small retry + host failover for 418/429/403/451."""
    global BASE_HOST
    last_exc = None
    for attempt in range(RETRY_ATTEMPTS):
        try:
            ...
            async with session.get(u, timeout=HTTP_TIMEOUT) as res:
                if res.status in (418, 429, 403, 451, 409):
...
async def choose_base_host(session):
    """Pick the first host that responds to /ping."""
    global BASE_HOST
...
# === ANALYSIS HELPERS (side-aware) ===
def indicator_votes(highs, lows, closes):
    """Signed votes across indicators: bullish=+1, bearish=-1, neutral=0."""
    votes = {}
    ...
    # PSAR: True means price above SAR (bullish), False means below (bearish)
    votes['SAR']  = +1 if psar_signal(highs, lows, closes) else -1

    # BOLL: breakout up/down only (neutral otherwise)
    c = np.asarray(closes, dtype=float)
    if len(c) >= 20:
        ma = c[-20:].mean(); sd = c[-20:].std(ddof=0)
        upper = ma + sd; lower = ma - sd
        if c[-1] > upper: votes['BOLL'] = +1
        elif c[-1] < lower: votes['BOLL'] = -1
        else: votes['BOLL'] = 0
    else:
        votes['BOLL'] = 0
...
def momentum_score_for_side(closes, volumes, side):
    """4-point momentum score aligned to the side (EMA9/21, RSI, MACD, + volume spike)."""
    e9 = ema(closes, 9); e21 = ema(closes, 21)
...
def action_hint(overall_trend, label, strength):
...
async def analyze_symbol(session, symbol, semaphore):
    async with semaphore:
        try:
            ...
            # indicator votes across 4h & 1d (signed)
            ...
            # require MID & HIGH trend agreement for pure lists
            ...
            classification = f"{overall_trend.capitalize()} {strength}" if label == "(Confirmed)" else f"{overall_trend.capitalize()} (M)"
            ...
            # indicator S/W relative to the chosen side
            def side_str(k):
                val = agg[k]
            ...
            act = action_hint(overall_trend, label, strength)
            msg = (
                f"**{symbol}** {tri}{tag_str} | {price_change:+.2f}% | Price: {last_price:.6g} | "
                f"Score:{side_score} | {label} | *{classification}* | {indicators_fmt} | Action: {act}"
            )

            # return tuple: (trend side,... message)
            return overall_trend, label, side_score, abs(price_change), msg
        except Exception as e:
...
async def scan_market(session, symbols):
    ...
    # StochRSI lists with your filters
    stoch_bull = sorted([r for r in stoch_filtered if r[0] == 'bullish' and r[1] >= STOCH_BULL_MIN_RISE], key=lambda x: -x[1])[:10]
    stoch_bear = sorted([r for r in stoch_filtered if r[0] == 'bearish' and r[1] <= -STOCH_BEAR_MIN_DROP], key=lambda x: x[1])[:10]
